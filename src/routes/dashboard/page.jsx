...
const DashboardPage = () => {
    ...
    // Vault data (replace with real state/blockchain data later)
    const vaultData = {
        wallet: 25,
        saved: 10,
        goal: 20,
    ...
    const getCurrentBadge = () => {
        if (vaultData.saved >= 50) return "Goal Crusher";
        if (vaultData.saved >= 25) return "Half Way";
        if (vaultData.saved >= 10) return "Vault Keeper";
        if (vaultData.saved >= 1)  return "First Steps";
        return "No Badge";
    };
    ...
    const handleWithdraw = () => {
        ...
        // Smart goal lock — funds locked until goal is reached
        if (vaultData.saved < goal) {
            showNotification("Funds are locked until goal is reached 🔒", "warning");
            setTransactions((prev) => [
                { id: Date.now(), type: "Withdraw (Locked)", amount },
                ...prev,
            ]);
            return;
        }
                    ...
                    <div className="flex items-center gap-10">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-semibold text-slate-100">{goal} ALGO</p>
                            </div>
                        </div>
                                ...
                                {transactions.map((tx) => (
                                    <tr key={tx.id} className="table-row">
                                        <td className="table-cell">
                                            <span className={tx.type.includes("Deposit") ? "text-green-500" : "text-red-500"}>
                                                {tx.type}
                                            </span>
                                        </td>
                                        <td className="table-cell">
